...
bool CSECOND::operator==(CSECOND left)
{
	if(*left.sign != *sign) return false;
	if(equal(left.str_num, str_num)) return true;
	return false;
...
bool CSECOND::operator!=(CSECOND left)
{
	if(*left.sign != *sign) return true;
	if(equal(left.str_num, str_num)) return false;
	return true;
...
{
	if(equal(left.str_num, str_num)) return false;
	if(*sign && !*left.sign) return true;
	if(!*sign && !*left.sign) return false;
	if(less(str_num,left.str_num)) return true;
	return false;
...
bool CSECOND::operator>(CSECOND left)
{
	...
	if(*sign && !*left.sign) return false;
	if(!*sign && !*left.sign) return true;
	if(greater(str_num,left.str_num)) return true;
	return false;
